...
> **Topology (2026-04-19):** **HarmonyForge is one Next.js app.** The SATB engine lives inside the same process at `frontend/src/server/engine/*` and is reached via `/api/*` route handlers (no separate Express on `:8000`, no `NEXT_PUBLIC_API_URL`, no CORS routing). A root `Dockerfile` bundles Node + pdfalto + Poppler + Python/oemer for self-hosted PDF OMR; Vercel serves everything except PDF OMR. See **[docs/deployment.md](../deployment.md)** for both targets.
>
...
- `Dockerfile` (repo root) + `docker-compose.yml` — Optional self-hosted image bundling Node + pdfalto + Poppler + Python/oemer for PDF OMR.
...
1. **Input**: User uploads **MusicXML (.xml/.musicxml), MXL, MIDI, or PDF** (dropzone). **`intakeFileToParsedScore`** (`frontend/src/server/engine/parsers/fileIntake.ts`): ZIP sniff (MXL mislabeled as `.xml`), then extension routing; **PDF** may use pdfalto → Poppler → **oemer** OMR (self-hosted Docker image) or client-rasterized PNG pages. Host needs **pdfalto** (often `miscellaneous/pdfalto/`), **Poppler**, **Python oemer** (`requirements.txt` at repo root, optional **`PDFALTO_BIN` / `POPPLER_PDFTOPPM` / `OEMER_BIN`**). See [deployment.md](../deployment.md) for Vercel vs Docker capability gaps.
...
8. **Export (2026-04-13):** From **`/sandbox`** **`ExportModal`** — all formats use **live** score after **`flushToZustand`** (**`getLiveScoreAfterFlush`**). **Client:** MusicXML (**`scoreToMusicXML`**), JSON (**`EditableScore`**), MIDI (**`scoreToMidi.ts`**), PNG (**`html-to-image`** on export preview viewport), WAV (**`Tone.Offline`** + **`scoreToWav.ts`**), ZIP (**`fflate`**: XML + MID + JSON + **`POST /api/export-chord-chart`** text). **Server:** chord chart from uploaded XML blob (same Roman chart as standalone export). **PDF:** **`window.print()`** with sandbox **`@media print`** (**`.hf-print-hide`** / **`.hf-sandbox-print-target`**). **Score Review** still **`POST /api/validate-from-file`** (engine re-solves SATB — not “validate my exact edits”). **Tablature** still deferred. See **`@progress.md` → [Tactile Sandbox exports](../progress.md#wl-sandbox-exports-2026-04-13)**.
