...
8. **Export (2026-04-13; OSMD PDF 2026-06-03):** From **`/sandbox`** **`ExportModal`** — all formats use **live** score after **`flushToZustand`**. **Client:** MusicXML, JSON, MIDI, PNG (RiffScore **`presentation`** preview), WAV, ZIP. **PDF/Print:** **`ExportPrintRoot`** → **`PrintableScore`** (**OpenSheetMusicDisplay**) + **`scoreToPartwiseMusicXML`** + **`window.print()`** with **`body.hf-printing-score`**. **Dual render paths:** OSMD is clef-accurate; RiffScore canvas depends on **`patch-package`** alto/tenor layout — if sandbox viola ≠ PDF, see **[Viola parity work log](../progress.md#wl-viola-sandbox-export-parity-2026-06-03)**. See also **[OSMD export + Audiveris](../progress.md#wl-export-audiveris-2026-06-03)**.
