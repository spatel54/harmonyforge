...
1. **Input**: User uploads **MusicXML (.xml/.musicxml), MXL, MIDI, or PDF** (dropzone). **`intakeFileToParsedScore`** (`frontend/src/server/engine/parsers/fileIntake.ts`): ZIP sniff (MXL mislabeled as `.xml`), then extension routing; **PDF** uses **Audiveris** batch OMR via **`audiverisPipeline.ts`** (self-hosted Docker or **`make audiveris-setup`**). Client **`pdfjs-dist`** raster is **preview-only** on Document. See [deployment.md](../deployment.md) for Vercel vs Docker capability gaps.
