...
**Code:** `harmony-forge-redesign/src/lib/study/studyConfig.ts`, `studyEventLog.ts`, `readMelodyXml.ts`, `components/study/*`, `vitest` `studyConfig.test.ts`. Env template: `harmony-forge-redesign/.env.example`.
...
3. **Theory Inspector (Explainability)** — LLM-powered validation *(M4 #79 MVP wiring complete)*
   ...
   - [x] **Note-inspector UX (2026-04):** `TheoryInspectorPanel` — plain-language titles + subtitles; **`aiSuggestions`** from tutor stream (`<<<SUGGESTIONS>>>`); `noteInsightAiSplit.ts` + vitest; `useTheoryInspectorStore.aiSuggestions`. **2026-04 follow-up:** panel order **Ideas → What the tool first wrote → What this click means → Verifiable export → Tutor summary** (summary last); deterministic **why** for axiomatic engine in **What this click means** (`buildAxiomaticEngineWhyParagraph`, additive path); tutor prompts require **what + why** on each suggestion bullet (`NOTE_EXPLAIN_TUTOR_BRIEF`, `prompts.ts` tutor).
...
- `cd harmony-forge-redesign && npm run test` — Vitest (`noteExplainContext`, `noteInsightAiSplit`, **`regionExplainContext`**, **`playbackScrub`**, **`studyConfig`**, and other frontend unit tests)
