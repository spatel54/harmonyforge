...
Skim **Status at a glance** below, then use the numbered **Plan** checklist for implementation detail. **Living narrative** (session notes, deep dives, regressions) lives in **[progress.md](progress.md)** — start with **[Program narrative (2026-04-20)](progress.md#program-narrative-2026-04-20)** for end goal, approach, shipped work, and **current focus** (deploy, study, residual risks — see narrative). **Architecture** diagram: **[context/system-map.md](context/system-map.md)**. **Full doc index:** **[docs/README.md](README.md)**. **Production deploy:** **[deployment.md](deployment.md)**.

### Status at a glance

- **Consolidation + PDF hardening (2026-04-19):** Backend + frontend merged into a single Next.js app. The SATB engine now lives at `frontend/src/server/engine/*` and is called directly by route handlers in `frontend/src/app/api/*` — no more Express proxy on `:8000`, no `NEXT_PUBLIC_API_URL`. Client rasterizes PDFs via `pdfjs-dist` (hook `useClientPdfPreview`) so Document always shows a preview; the engine now accepts pre-rasterized PNGs and merges multi-page PDFs (`mergeParsedScores`), closing §1.9m's "page 1 only" footnote. A root `Dockerfile` bundles Node + Next + pdfalto + Poppler + Python/oemer for self-hosted PDF OMR. Lint is green (0 warnings), `make verify` runs 190 Vitest tests + lint + build. Theory Inspector now auto-wires tutor `<<<INTENT>>>` JSON to store setters + `router.push` (mood/genre/pickup/regenerate/navigate) via `intentRouter.ts`. `ideaActionResolve` gained `staffIndex` disambiguation + nearest-in-measure fallback. ADR 003 vertical slice shipped — Bb clarinet / F horn / Eb alto sax emit proper `<transpose>` in MusicXML. Audio unlock banner on Sandbox mitigates "inaudible first note" Iter2 §4.
- **Study refinement (2026-04-18):** Comprehensive pass addressing every friction point in [docs/iterations.md](iterations.md) — tenor clef fix, rhythm density (`chordal`/`mixed`/`flowing`), voice-independence bias, anacrusis auto-detection, persistent Document config, Inspector starter prompts + musical goal + honest INTENT block, sandbox delete-to-rest, labeled toolbar, volume slider, playback resilience. See [progress.md — Work log — Iteration 1+2 study refinement (2026-04-18)](progress.md#wl-study-refinement-2026-04-18).
...
- **Program state (2026-04-20):** Single-page summary of end goal, approach, shipped work, and **current focus** — [progress.md — Program narrative](progress.md#program-narrative-2026-04-20).
...
- **First-run UX:** With **`COACHMARKS_ENABLED`**, the **coachmark tour** auto-starts on **`/`** (modal/coachmark cards disabled); otherwise **`OnboardingModal`** until dismissed. **`llmClient`** can use optional OpenAI-compatible base URL env vars when set (see [deployment.md](deployment.md)).
