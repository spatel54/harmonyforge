...
> **Current status:** Logic Core complete (1.1–1.9). Tactile Sandbox editing is **RiffScore-first** (`harmony-forge-redesign/`): `EditableScore` in Zustand syncs bidirectionally with RiffScore; rests normalized; **riffscore** extended via **`patch-package`** (`ui.toolbarPlugins`) for native-toolbar HarmonyForge actions. Document preview may still use parsed score / RiffScore preview. **Additive harmonies:** Engine adds harmony parts to melody. **API/CLI:** Partwise MusicXML 2.0; `make test-engine`. **Flow:** Upload → Document → Generate → Sandbox. Session persistence via sessionStorage; CORS via `CORS_ORIGIN`. **Theory Inspector:** `/api/theory-inspector` + `/api/theory-inspector/suggest`; **dual-mode** (Origin Justifier / Harmonic Guide / melody-context) behind plain-language UI; **note-click:** **Tutor summary** + **Ideas to try next** (`<<<SUGGESTIONS>>>` split via `noteInsightAiSplit.ts`, `NoteInsight.aiSuggestions`); deterministic blocks reordered and relabeled for average users; **canvas staff labels** (`extractStaffLabelLayout`, `RiffScoreEditor` overlays + fallback list). RAG from **`Taxonomy.md`** + **`taxonomyIndex.ts`**; **`prompts.ts`** — **`CITATION_AND_BREVITY`** + **`HONESTY_NO_SYCOPHANCY`**. Baseline + `originalGeneratedPitch`; **staff roster + cross-part interval FACTs** in `noteExplainContext.ts`; SATB note-explain when **exactly four parts**. Optional OpenAI (`.env.local`). **Tests:** vitest includes `noteExplainContext`, `noteInsightAiSplit`. **Active gaps:** RiffScore `/audio/piano/*.mp3` 404s; Turbopack multi-lockfile; `runAudit` vs 5+ staves; Mode A rationale thin; **panel autoscroll** uses **`useLayoutEffect` without deps** to avoid React Compiler changing `useEffect` dependency array length under Turbopack—revisit when a compiler-safe pattern is confirmed; full `npm run lint` still has legacy debt.
>
...
- `cd harmony-forge-redesign && npm run test` — Vitest (`noteExplainContext`, `noteInsightAiSplit`, and future frontend unit tests)
