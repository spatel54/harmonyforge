...
2. **Verify the flow.** The user journey must work:
   - **Step 1 (Playground)**: Upload music (XML, MIDI) via `frontend/src/app/page.tsx` → file stored in `useUploadStore` → navigate to `/document`.
...
4. **Fix errors iteratively.** If you encounter:
   - Parse errors → fix `frontend/src/lib/music/musicxmlParser.ts` or backend parsers.
   - VexFlow empty canvas → fix `frontend/src/components/score/VexFlowScore.tsx` (BarNote between measures, `voice.setStrict(false)`).
   - API errors → fix `backend/engine/server.ts` or `backend/engine/parsers/`.
...
6. **Manual test checklist.** Document that the user can:
   - Start backend: `cd backend && npm run dev:backend` (port 8000)
