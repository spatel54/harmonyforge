...
function harmonyToRoman(harmony) {
...
export function parseMusicXML(xml) {
    ...
    if (hasTimewise) {
        ...
        if (timewise && timewise.melody.length > 0)
            return timewise;
    }
    if (hasPartwise) {
        const partwise = parsePartwiseMusicXML(xml);
    ...
    try {
            ...
            for (const el of elements) {
                ...
                if ("backup" in el) {
                    ...
                    if (typeof backup === "number")
                        currentBeat -= backup / divisions;
                }
                if ("forward" in el) {
                    const forward = el.forward;
                    if (typeof forward === "number")
                        currentBeat += forward / divisions;
                }
                if ("root" in el || "function" in el) {
                    const roman = harmonyToRoman(el);
                    if (roman)
                        chordSlots.push({ roman, beat: currentBeat });
                }
            }
        }
        ...
        if (melodyNotes.length === 0) {
            if (hasTimewise)
                return parseTimewiseMusicXML(xml);
