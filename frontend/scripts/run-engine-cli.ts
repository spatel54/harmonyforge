/**
 * CLI to run HarmonyForge engine: input score file → output MusicXML with melody + harmonies.
 *
 * Usage (from frontend/):
 *   npx tsx scripts/run-engine-cli.ts
 *   npx tsx scripts/run-engine-cli.ts --input path/to/foo.xml --output path/to/out.xml
 ...
 *
 * Options:
 *   -i, --input <path>    Input .xml, .musicxml, .mxl, .mid/.midi, or .pdf
 *                         (default: frontend/public/samples/tour_demo.xml)
 *   -o, --output <path>   Output path (default: frontend/engine-cli-output/<name>.xml)
...
import { intakeFileToParsedScore } from "../src/server/engine/parsers/fileIntake";
import { ensureChords } from "../src/server/engine/chordInference";
import { generateSATB, SolverBudgetExceededError } from "../src/server/engine/solver";
import { satbToMusicXML } from "../src/server/engine/satbToMusicXML";
import type { Voice } from "../src/server/engine/types";

const SCRIPT_DIR = dirname(fileURLToPath(import.meta.url));
const FRONTEND_ROOT = join(SCRIPT_DIR, "..");
const DEFAULT_INPUT = join(FRONTEND_ROOT, "public", "samples", "tour_demo.xml");
const DEFAULT_OUTPUT_DIR = join(FRONTEND_ROOT, "engine-cli-output");
...
function parseArgs(): {
  ...
  if (!output) {
    ...
    output = join(DEFAULT_OUTPUT_DIR, `${base}_${suffix}.xml`);
  }
