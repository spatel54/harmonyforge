...
describe("solver", () => {
  ...
  it("prefers independent S/A voicings over pervasive octave doubling (Iter1 §2)", () => {
    const melody = [
      { pitch: "C5", beat: 0 },
      { pitch: "D5", beat: 1 },
      { pitch: "E5", beat: 2 },
    ...
    const chords = melody.map((m, i) => ({ roman: i % 2 === 0 ? "I" : "V", beat: m.beat }));
    const result = generateSATB({
      key: { tonic: "C", mode: "major" },
      chords,
      melody,
    ...
    const pitchToMidi = (p: string): number => {
      const match = p.match(/^([A-G]#?)(-?\d+)$/)!;
      const pc = ["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"].indexOf(match[1]);
      return 12 * (parseInt(match[2], 10) + 1) + pc;
    };
    const doublingCount = result!.slots.filter((s) => {
      return mod12(pitchToMidi(s.voices.soprano)) === mod12(pitchToMidi(s.voices.alto));
    }).length;
    expect(doublingCount).toBeLessThan(result!.slots.length);
  });
