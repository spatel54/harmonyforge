...
interface PartwiseMelodyExtraction {
  ...
  /** Anacrusis / pickup length in beats at start of part; 0 when no pickup is detected. */
  pickupBeats: number;
}
...
function extractMelodyFromMeasures(measures: Record<string, unknown>[]): PartwiseMelodyExtraction | null {
  ...
  for (let measureIdx = 0; measureIdx < measures.length; measureIdx++) {
    ...
    if (measureIdx === 0) {
      const implicit = typeof m["@_implicit"] === "string" && String(m["@_implicit"]).toLowerCase() === "yes";
      const measureNumberAttr = m["@_number"];
      const numberIsZero = typeof measureNumberAttr === "string"
        ? measureNumberAttr.trim() === "0"
        : measureNumberAttr === 0;
      const measureDurationBeats = currentBeat - measureStartBeat;
      const isShortFirstMeasure = measureDurationBeats > 0 && measureDurationBeats + 1e-4 < timeSignature.beats;
      if (implicit || numberIsZero || isShortFirstMeasure) {
        pickupBeats = measureDurationBeats;
      }
    }
  }
