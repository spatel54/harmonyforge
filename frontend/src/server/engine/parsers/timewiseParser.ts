...
export function parseTimewiseMusicXML(xml: string): ParsedScore | null {
  try {
    ...
    for (let mIdx = 0; mIdx < measures.length; mIdx++) {
      ...
      if (mIdx === 0) {
        const implicit = typeof measure["@_implicit"] === "string"
          && String(measure["@_implicit"]).toLowerCase() === "yes";
        const measureNumberAttr = measure["@_number"];
        ...
        const isShortFirstMeasure = measureDurationBeats > 0
          && measureDurationBeats + 1e-4 < timeSignature.beats;
    ...
    return {
      ...
      melodyPartName,
      pickupBeats: pickupBeats > 0 ? pickupBeats : undefined,
    };
  } catch {
    return null;
  }
