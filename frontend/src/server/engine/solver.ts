...
import { midiToPitch, pitchToMidi } from "./types";
import type { LeadSheet, MelodyNote, ParsedChord, SATBVoices, Genre } from "./types";
import { parseChord } from "./chordParser";
import { checkVoiceLeading, checkVoiceLeadingRelaxed } from "./constraints";
import { getVoiceRange } from "./types";
...
/**
 * Candidate scoring — lower is preferred when sorting.
 * Rewards smooth voice leading (Aldwell & Schachter §5 on "shortest path")
 * AND penalizes voicings that double the soprano melody (per Iter1 §2 study
 * feedback about S/A sounding "in unison for the entire duration"):
 *   - hard unison S==A (cannot actually occur today because getCandidatesSimple
 *     requires a < s, but we guard in case constraints are ever relaxed)
 *   - parallel octave doubling S-A (pitch class equal): strong bias away
 *   - same-direction, same-distance S/A motion relative to prev (parallel motion):
 *     moderate bias away to encourage independence over multi-slot spans.
 */
function candidateMotionScore(prev: SATBVoices | null, curr: SATBVoices): number {
  ...
  if (prev) {
    for (const k of keys) {
      total += Math.abs(pitchToMidi(curr[k]) - pitchToMidi(prev[k]));
    }
  }

  const sCur = pitchToMidi(curr.soprano);
  const aCur = pitchToMidi(curr.alto);
  if (sCur === aCur) {
    total += 80;
  } else if (mod12(sCur) === mod12(aCur)) {
    total += 18;
  }

  if (prev) {
    const sPrev = pitchToMidi(prev.soprano);
    const aPrev = pitchToMidi(prev.alto);
    const sDelta = sCur - sPrev;
    const aDelta = aCur - aPrev;
    if (sDelta !== 0 && sDelta === aDelta) {
      total += 6;
    }
  }
