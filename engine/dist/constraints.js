 ...
 */
import { pitchToMidi, inRange, } from "./types.js";
...
function isPerfectFifth(a, b) {
    return intervalSemitones(a, b) === 7;
}
function isPerfectOctaveOrUnison(a, b) {
...
const VOICE_KEYS = ["soprano", "alto", "tenor", "bass"];
/** Check all voices are in range */
export function checkRange(voices) {
    for (let i = 0; i < VOICES.length; i++) {
        const pitch = voices[VOICE_KEYS[i]];
        if (!inRange(VOICES[i], pitch))
            return false;
    }
    return true;
}
/** Spacing: S–A ≤ octave (12), A–T ≤ octave, T–B ≤ twelfth (19) */
export function checkSpacing(voices) {
...
export function checkVoiceOrder(voices) {
    const s = pitchToMidi(voices.soprano);
    const a = pitchToMidi(voices.alto);
    const t = pitchToMidi(voices.tenor);
    const b = pitchToMidi(voices.bass);
...
export function hasParallelFifth(prev, curr) {
    const pairs = [
        ["soprano", "alto"],
        ["soprano", "tenor"],
        ["soprano", "bass"],
...
export function hasParallelOctave(prev, curr) {
    const pairs = [
        ["soprano", "alto"],
        ["soprano", "tenor"],
        ["soprano", "bass"],
    ...
    for (const [v1, v2] of pairs) {
        ...
        const p2a = pitchToMidi(curr[v1]);
        const p2b = pitchToMidi(curr[v2]);
        if (isPerfectOctaveOrUnison(p1a, p1b) &&
...
export function hasVoiceOverlap(prev, curr) {
    ...
    for (let i = 0; i < 4; i++) {
        for (let j = i + 1; j < 4; j++) {
            ...
            if (currMidi[j] > prevMidi[i])
                return true;
            if (currMidi[i] < prevMidi[j])
                return true;
        }
    }
    return false;
...
export function checkChordConstraints(voices) {
 ...
 */
export function checkVoiceLeading(prev, curr) {
    if (!checkChordConstraints(curr))
        return false;
    if (prev === null)
        return true;
...
export function checkVoiceLeadingRelaxed(prev, curr) {
    if (!checkChordConstraints(curr))
        return false;
    if (prev === null)
        return true;
