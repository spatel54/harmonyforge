...
app.post("/api/generate-from-file", upload.fields([
  ...
  const configRaw = (req.body as { config?: string })?.config;
  ...
  if (ext === "pdf") {
    ...
    parsed = parseMXL(file.buffer as Buffer);
  } else if (["mid", "midi"].includes(ext)) {
    parsed = parseMIDI(file.buffer as Buffer);
  } else {
    res.status(400).json({
