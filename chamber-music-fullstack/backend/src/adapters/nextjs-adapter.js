/**
 * Complete harmonization logic ported from TypeScript to JavaScript
 * Adapted from Next.js route handler to work in Express environment
 */
...
function querySelectorAll(element, tagName) {
  const elements = element.getElementsByTagName(tagName);
...
/**
 * @typedef {Object} Note
 * @property {number} pitch - MIDI pitch number (-1 for rest)
 * @property {number} duration - Note duration
 * @property {number} offset - Time offset in the piece
 */

/**
 * @typedef {Object} Chord
 * @property {number} root - MIDI pitch of chord root
 * @property {"major"|"minor"|"diminished"|"augmented"} quality - Chord quality
 * @property {0|1|2} inversion - 0 = root position, 1 = first inversion, 2 = second inversion
 * @property {number[]} voices - MIDI pitches for each voice [soprano, alto, tenor, bass]
 */

/**
 * @typedef {Object} VoiceLeadingContext
 * @property {Chord|null} previousChord - Previous chord in progression
 * @property {number|null} previousMelody - Previous melody note
 * @property {number} measurePosition - Position within measure
 * @property {number} phrasePosition - Position within phrase
 * @property {number} instrumentVariation - Variation factor for instrument
 */

/**
 * @typedef {Object} InstrumentConfig
 * @property {"G"|"F"|"C"} clefSign - Clef sign
 * @property {2|3|4} clefLine - Clef line
 * @property {number} minMidi - Minimum MIDI pitch
 * @property {number} maxMidi - Maximum MIDI pitch
 * @property {number} transposition - Semitones up for written pitch
 */

/**
 * @typedef {Object} HarmonicAnalysis
...
// ============================================================================
// MOCK NEXTRESPONSE FOR EXPRESS COMPATIBILITY
// ============================================================================
...
export async function POST(request) {
  ...
  try {
    ...
    const instrumentsStr = formData.get("instruments") || "Violin";
...
async function harmonizeMelody(xmlContent, instruments) {
...
function detectPolyphony(xmlDoc) {
...
async function harmonizeMonophonic(xmlDoc, instruments, root, scale, mode, fifths, rng) {
  ...
  const instrumentVoiceMappings = {};
  const voiceOrder = [1, 3, 2];
  instruments.forEach((instrument, index) => {
    instrumentVoiceMappings[instrument] = voiceOrder[index % 3];
  });

  const harmoniesByInstrument = {};
...
async function harmonizePolyphonic(xmlDoc, instruments, root, scale, mode, fifths, rng) {
...
function extractNotesPolyphonic(xmlDoc) {
...
function generateHarmonicProgression(melodyNotes, root, scale, mode, enableVariation, rng) {
...
function generateHarmonicProgressionPolyphonic(melodicLines, root, scale, mode, enableVariation, rng) {
  const scalePitches = scale.map((interval) => (root + interval) % 12);
  const chords = [];
...
function selectChordDegree(melodyScaleDegree, isStrongBeat, isEndOfPhrase, context, variation = 0, rng) {
  if (isEndOfPhrase) {
...
function getChordQuality(scaleDegree, isMajorKey) {
  if (isMajorKey) {
...
function voiceChord(melodyPitch, chordRoot, chordThird, chordFifth, inversion, context) {
  ...
  let altoTone, tenorTone, bassTone;
...
function findClosestPitch(pitchClass, targetOctave, minMidi, maxMidi) {
...
function validateHarmonicProgression(chords, melodyNotes, keyRoot, scale, isMajor) {
  const warnings = [];
  let score = 100;
...
function refineHarmonicProgression(chords, melodyNotes, keyRoot, scale, isMajor) {
  const refined = [];
...
function findBestInversionForCommonTone(currentChord, previousChord, keyRoot, scale, isMajor) {
...
function smoothVoiceLeading(currentChord, previousChord) {
...
// ============================================================================
// INSTRUMENT PART GENERATION
// ============================================================================

function generateInstrumentPart(harmonicProgression, assignedVoiceIndex, instrumentConfig, melodyNotes) {
  const instrumentNotes = [];
  let previousVoicePitch = null;
...
function getKeyInfo(fifths, mode) {
...
function createMultiInstrumentHarmonyXML(originalDoc, harmoniesByInstrument) {
...
function createCombinedMultiInstrumentXML(originalDoc, melodyNotes, harmoniesByInstrument) {
  const fifths = querySelector(originalDoc, "fifths")?.textContent || "0";
  const mode = querySelector(originalDoc, "mode")?.textContent || "major";
  const beats = querySelector(originalDoc, "beats")?.textContent || "4";
  const beatType = querySelector(originalDoc, "beat-type")?.textContent || "4";
...
function createCombinedPolyphonicXML(originalDoc, melodicLines, harmoniesByInstrument, fifths, mode) {
  const beats = querySelector(originalDoc, "beats")?.textContent || "4";
  const beatType = querySelector(originalDoc, "beat-type")?.textContent || "4";
  const divisions = querySelector(originalDoc, "divisions")?.textContent || "1";
...
function createPartXML(partId, notes, fifths, mode, beats, beatType, divisions, instrumentConfig) {
  let xml = `
  <part id="${partId}">
    <measure number="1">
      <attributes>
